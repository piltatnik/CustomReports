...
/*                    AND ((nvl(trans.new_card_series, trans.card_series) NOT IN
  ...
  BEGIN
      ...
      select count(1) into vPrivilegeCount  from cptt.privilege where code LIKE '000000__' or code LIKE '100000__';
...
trans_grouped_second as(     
     ...
     (select ct.key, ct.parent_key, ct.caption, 
            ...
            sum(case when (ct.trans_type='ACTIVE')  AND (series in ('31', '32', '34', '35', '36', '39', '53', '43', '46')) then count_trans else null end) as count_active_ak,
            sum(case when (ct.trans_type='ACTIVE')  AND (series in ('21', '22', '24', '25', '26', '29', '52', '42', '45')) then count_trans else null end) as count_active_urt,
     ...
     where key != '3.'
     union all
     select sum(1), null as key, null as parent_key, 'Итого' as caption,
            sum(case when key IN ('1.', '2.', '4.') then count_active_all else 0 end), null as count_active_other, null as count_active_ak, null as count_active_urt,
            sum(case when key IN ('1.', '2.', '4.') then count_pass_all else 0 end), sum(case when key IN ('1.', '2.', '4.') then count_pass_other else 0 end), sum(case when key IN ('1.', '2.', '4.') then count_pass_ak else 0 end), sum(case when key IN ('1.', '2.', '4.') then count_pass_urt else 0 end),
     ...
     (select ct.key, ct.parent_key, ct.caption, car.id_operator, car.id_division,
            sum(case 
                    when ct.trans_type='ACTIVE' and series is not null and series not in ('96')
                      then case
                             when car.id_operator in (500246845) AND (series not in ('21', '22', '24', '25', '26', '29', '52', '42', '45') or series is null) then count_trans 
                             when car.id_operator in (400246845) AND (series not in ('31', '32', '34', '35', '36', '39', '53', '43', '46') or series is null) then count_trans
                             when car.id_operator not in (400246845, 500246845) AND (series not in ('21', '22', '24', '25', '26', '29', '52', '42', '45') /*or series is null*/) then count_trans
                             else 0
                           end
             ...
             sum(case 
                    when ct.trans_type='ACTIVE' and series is not null and series not in ('96')
                      then case
                             when car.id_operator in (500246845) AND (series not in ('21', '22', '24', '25', '26', '29', '52', '42', '45') or series is null) then sum_trans 
                             when car.id_operator in (400246845) AND (series not in ('31', '32', '34', '35', '36', '39', '53', '43', '46') or series is null) then sum_trans
     ...
     from contents_trans ct
       ...
       (select key, parent_key, caption, id_operator, id_division,
              ...
              case
                when key in ('4.') then
                  ...
                  sum_total
              end * 0.0298 as sum_total_investor 
       ...
       select list_num,
              ...
              cptt.pkg$trep_reports.getExcelColName(cols.num) as col_name,
              case cols.num
                when 1 then (case when key in ('1.', '1.1.', '1.2.', '1.3.', '2.', '2.1.', '2.2.', '3.', '4.') then decode(key, '4.', '2.', key) else null end)
                when 2 then lpad(' ', length(key)*2, ' ') ||caption
        ...
        select lists.list_num as list_num,
               ...
               'A' as col_name,
               case vals.num
                 when 1 then 'Инвестор-Оператор          _______________  __________________  _____________'
      ...
      for rec_format in (
        ...
        select 1 as list_num, 
               cptt.pkg$trep_reports.getRange('A', 5, 'N', 4 + 34 + vPrivilegeCount) as range,
               1 as border,
               'N' as is_merged
        from dual
        union all
        SELECT list_num,
               cptt.pkg$trep_reports.getRange('A', 5, case 
                    ...
                    SELECT 1 AS list_num,
                           ...
                           CASE percent_rows.num
                             ...
                             ELSE
                              CASE percent_cols.col_num
                                WHEN 1 THEN
                                 to_char(round(4 / 10, 2))
                                ELSE
                                 '=1-A' || to_char(percent_rows.row_num)
                              END
                           END AS VALUE
