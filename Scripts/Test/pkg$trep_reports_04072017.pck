...
/*                    AND ((nvl(trans.new_card_series, trans.card_series) NOT IN
  ...
  BEGIN
    ...
    AND trans.date_of < pPassEndDate
    AND trans.d = 0
    AND trans.kind IN ('1', '2', '32', '14', '17', '20')
  ...
  BEGIN
    ...
    FROM operator op
    WHERE op.role = 1
    AND op.id NOT IN (select id from cptt.ref$trep_agents_locked/*2200246845,
                     4100246845, \*маршрутки сейчас исключены*\
                     16100246845*/);
    ...
    WITH carrier AS
     (SELECT id AS id_operator
      FROM operator
      WHERE role = 1
      AND id NOT IN (select id from cptt.ref$trep_agents_locked/*2200246845,
                    4100246845, \*маршрутки сейчас исключены*\
                    16100246845*/))
    SELECT COUNT(1)
    INTO vRouteCount
