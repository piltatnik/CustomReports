...
from 
  (select distinct sc.id_code,coalesce(t_data.new_card_series,t_data.card_series) as c1,count(*) as sh,avg(t_data.amount) as av,sum(t_data.amount) as summa   
    ...
    where coalesce(t_data.new_card_series,t_data.card_series) in (11,12,13) and t_data.kind in (7,8,12,13) and t_data.id_division in  (800246845,700246845,6100246845,6200246845,8100246845)                               
...
(select sc.id_code,coalesce(t_data.new_card_series,t_data.card_series) as c1,count(*) as sh,sum(t_data.amount) as summa   
    from CARD left outer join t_data on  card.id=t_data.id_card 
    left join ser_code1 sc on coalesce(t_data.new_card_series,t_data.card_series)=sc.id_ser
    where coalesce(t_data.new_card_series,t_data.card_series) in (21,22) and t_data.kind in (7,8,12,13) and t_data.id_division  in  (800246845,700246845,6100246845,6200246845,8100246845)               
    and t_data.date_of between '13.12.2016' and '06.01.2017'                                                    
...
full outer join 
    ...
    from CARD left outer join t_data on  card.id=t_data.id_card 
    left join ser_code1 sc on coalesce(t_data.new_card_series,t_data.card_series)=sc.id_ser
    where coalesce(t_data.new_card_series,t_data.card_series) in (31,32) and t_data.kind in (7,8,12,13) and t_data.id_division  in  (800246845,700246845,6100246845,6200246845,8100246845)                
    and t_data.date_of between '13.12.2016' and '06.01.2017'         
