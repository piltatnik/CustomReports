select s02.d as "Дата" ,s02.train_table as "Табельный номер",coalesce(s0.sh,0) as "Проезд за наличные",
...
from 
    ...
    and t_data.date_of between '01.01.2017' and '15.01.2017' 
   -- group by to_date(t_data.date_of,'dd-MM-YYYY'),t_data.train_table,
    ) s02    
 left outer join 

  ( select to_date(t_data.date_of-3/24,'dd-MM-YYYY') as d,t_data.train_table,coalesce(card.series,'02') as c,count(t_data.id) as sh   
    ...
    group by to_date(t_data.date_of-3/24,'dd-MM-YYYY'),t_data.train_table,coalesce(card.series,'02')
...
left outer join 
  ( select to_date(t_data.date_of-3/24,'dd-MM-YYYY') as d, t_data.train_table,t_data.card_series,count(t_data.id) as sh  
    from t_data left join card on t_data.id_card=card.id
    ...
    group by to_date(t_data.date_of-3/24,'dd-MM-YYYY'),t_data.train_table,t_data.card_series
...
(select to_date(t_data.date_of-3/24,'dd-MM-YYYY') as d,t_data.train_table,count(t_data.id) as sh   
    ...
    group by to_date(t_data.date_of-3/24,'dd-MM-YYYY'),t_data.train_table
    ) s22
  on 
