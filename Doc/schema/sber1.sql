...
select distinct c.num,replace(c.f,' ',',') as "fio",
       case when sc.ser=17 then p.name --если серия 17,то смотрим название льготы по таблице privelege
            else upper(sc.ser_name) end ,
       case when extract(day from sysdate)<13                 -- выводим месяц,на который можно купить проездной
       ...
       end as "month",'Проездной',sc.amount
from t_data t right join card c  
  ...
  and sc.ser not in (13,16)                                 -- исключаем студентов и школьников 100%                    
  and c.num not in                                          -- фильтр на карты , у которых есть проездной
      (select c.num                                             
       ...
       t.date_to=                             --вычисляем на какой месяц можно купить проездной
                     (select case when extract(day from sysdate)<13             --до 13 числа    
                      then last_day(trunc(sysdate))                       -- на текущий месяц
